...
/*
 * ASUS Zenbook 14 UX3405CA (SSID 1043:1A63) already has a complete _DSD:
 * external boost, shared reset GPIO, SPI CS, gpio2 = IRQ. Two ACPI bugs remain:
 *  - no spk-id-gpios (CRS GPIO index 2)
 *  - cirrus,gpio1-func is VSPK on the left amp and unused on the right, so the
 *    right channel binds with VSPK: 0 and volume drops ~1s after playback.
 *
 * generic_dsd_config must not be used here: it ignores the existing _DSD and
 * leaves the right amp failing SPI after some S3 resumes (PM -110 / EBUSY -16).
 */
static int asus_ux3405ca_config(struct cs35l41_hda *cs35l41, struct device *physdev, int id,
				const char *hid)
{
	int ret;

	ret = missing_speaker_id_gpio2(cs35l41, physdev, id, hid);
	if (ret)
		return ret;

	if (cs35l41->hw_cfg.gpio1.func == CS35L41_NOT_USED) {
		dev_info(cs35l41->dev,
			 "UX3405CA: enabling VSPK switch on GPIO1 (ACPI left it unused)\n");
...
static const struct cs35l41_prop_model cs35l41_prop_model_table[] = {
	...
	{ "CSC3551", "10431A63", asus_ux3405ca_config },
