...
static int custom_amp_gain = 20;
module_param(custom_amp_gain, int, 0644);
MODULE_PARM_DESC(custom_amp_gain, "Custom Hardware Amplifier Gain (0 to 20, default=20 for max volume)");
...
static const struct reg_sequence cs35l41_hda_unmute[] = {
	...
	{ CS35L41_AMP_GAIN_CTRL,	0x0000028C }, // AMP_GAIN_PCM 20.5 dB Max Hardware Gain
};
...
{
	cs35l41->tuning_gain = clamp(custom_amp_gain, 0, CS35L41_AMP_GAIN_PCM_MAX);
}
...
static int cs35l41_system_resume(struct device *dev)
{
	...
	/*
	 * Software reset after a hardware reset is redundant and, with a
	 * shared reset GPIO, makes the second amp miss OTP_BOOT_DONE (-110).
	 * Only software-reset when no reset GPIO is available.
